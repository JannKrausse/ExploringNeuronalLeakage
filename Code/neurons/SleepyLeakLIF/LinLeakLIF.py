...
class LinLeakLIF(IntegratorCell):
    def __init__(self, n_in, n_rec, base_leak, thr, dt,
                 recurrence=False, fixed=False, **kwargs):
        super(LinLeakLIF, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
    ...
    def __call__(self, inputs, state, training=False):
        ...
        new_v = new_v - tf.sign(new_v) * self.base_leak * tf.minimum(self.dt * h * (new_t + 1),
                                                                     tf.sign(new_v) / (self.base_leak + 1e-8))
...
class LinLeakyIntegrator(StdLeakyIntegrator):
    def __init__(self, n_in, n_rec, base_leak, dt, recurrence=False, fixed=False, **kwargs):
        super(LinLeakyIntegrator, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, dt=dt, tau=None, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
        self.base_leak = base_leak
        self.dt = dt
