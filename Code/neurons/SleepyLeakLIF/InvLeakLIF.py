...
class InvLeakLIF(IntegratorCell):
    def __init__(self, n_in, n_rec, a_inv, b_inv, thr, dt,
                 recurrence=False, fixed=False, **kwargs):
        super(InvLeakLIF, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
        self.a_inv = a_inv
        self.b_inv = b_inv
    ...
    def __call__(self, inputs, state, training=False):
        ...
        new_v = new_v * (1 - (self.a_inv * tf.minimum(self.dt * h * (new_t + 1),
                                                tf.constant(1. / self.a_inv))) ** self.b_inv)  # new dt
...
class InvLeakyIntegrator(StdLeakyIntegrator):
    def __init__(self, n_in, n_rec, a_inv, b_inv, dt, recurrence=False, fixed=False, **kwargs):
        super(InvLeakyIntegrator, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, dt=dt, tau=None, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
        self.a_inv = a_inv
        self.b_inv = b_inv
