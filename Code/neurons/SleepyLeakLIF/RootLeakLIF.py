...
class RootLeakLIF(IntegratorCell):
    def __init__(self, n_in, n_rec, a_root, b_root, thr, dt,
                 recurrence=False, fixed=False, **kwargs):
        super(RootLeakLIF, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
        self.a_root = a_root
        self.b_root = b_root
    ...
    def __call__(self, inputs, state, training=False):
        ...
        new_v += -(1. - low_pass) * (v + 1.)

        new_v = new_v * (1 / self.a_root ** (2 * self.b_root) * (
                    tf.minimum(self.dt * h * (new_t + 1), tf.constant(self.a_root)) - self.a_root) ** (
                                             2 * self.b_root))
        # one variant with cap and one w/o???
...
class RootLeakyIntegrator(StdLeakyIntegrator):
    def __init__(self, n_in, n_rec, a_root, b_root, dt, recurrence=False, fixed=False, **kwargs):
        super(RootLeakyIntegrator, self).__init__(n_in=n_in, n_rec=n_rec, fixed=fixed, dt=dt, tau=None, **kwargs)
        self.n_in = n_in
        self.n_rec = n_rec
        self.a_root = a_root
        self.b_root = b_root
